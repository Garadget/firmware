...
void MQTT::init(uint8_t *ip, uint16_t port, CallbackDelegate cbDelegate
#if defined(SPARK) || (PLATFORM_ID==88)
#elif defined(ARDUINO)
        , Client& client
#endif
...
#if defined(SPARK) || (PLATFORM_ID==88)
    this->initialize(NULL, ip, port, cbDelegate, MQTT_MAX_PACKET_SIZE);
#elif defined(ARDUINO)
    this->initialize(NULL, ip, port, cbDelegate, MQTT_MAX_PACKET_SIZE, client);
#endif
}
...
//this function below is the same as the above function, but updated to use delegates
void MQTT::initialize(char* domain, uint8_t *ip, uint16_t port, CallbackDelegate cbDelegate, int maxpacketsize
#if defined(SPARK) || (PLATFORM_ID==88)
#elif defined(ARDUINO)
        , Client& client
#endif
    ...
    this->callback = NULL;
    this->callbackDelegate = cbDelegate;
    this->callbackIsSet = true;
                    ...
                    if (callbackIsSet) {
                        ...
                        if ((buffer[0]&0x06) == MQTTQOS1_HEADER_MASK) {
                            ...
                            if (callback) {
                               ...
                               callbackDelegate(topic,payload,len-llen-3-tl-2);
                            }
                            ...
                            if (callback) {
                                ...
                                callbackDelegate(topic,payload,len-llen-3-tl);
                            }
